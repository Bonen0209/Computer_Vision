...
def get_dataloader_vgg(folder, batch_size=32, num_workers=0, feature=False):
    ...
    if feature:
        train_loader = DataLoader(dataset=train_set, batch_size=batch_size, shuffle=False, num_workers=num_workers)
    else:
        train_loader = DataLoader(dataset=train_set, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    
    test_loader  = DataLoader(dataset=test_set,  batch_size=batch_size, shuffle=False, num_workers=num_workers)

    print (f'==>>> total trainning batch number:\t{len(train_loader)}')
